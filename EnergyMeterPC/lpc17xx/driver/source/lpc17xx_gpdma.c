 ...
 */
static const LPC_GPDMACH_TypeDef *pGPDMACh[] = {
 ...
 *********************************************************************/
Status GPDMA_Setup(const GPDMA_Channel_CFG_Type *GPDMAChannelConfig)
{
	...
	case GPDMA_TRANSFERTYPE_P2M:
		...
		break;
	// Peripheral to memory
	case GPDMA_TRANSFERTYPE_M2GPIO:
		pDMAch->DMACCSrcAddr = GPDMAChannelConfig->SrcMemAddr;
		pDMAch->DMACCDestAddr = GPDMAChannelConfig->DstMemAddr;
	...
	uint32_t type = GPDMAChannelConfig->TransferType;
	if(type == GPDMA_TRANSFERTYPE_M2GPIO) {
		type = GPDMA_TRANSFERTYPE_M2P;
	}

	// Configure DMA Channel, enable Error Counter and Terminate counter
	pDMAch->DMACCConfig = GPDMA_DMACCxConfig_IE | GPDMA_DMACCxConfig_ITC /*| GPDMA_DMACCxConfig_E*/ \
		| GPDMA_DMACCxConfig_TransferType(type) \
